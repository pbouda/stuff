    ...
    if skeletons is not None:
        for index, data in enumerate(skeletons):
            if (data.eTrackingState == nui.SkeletonTrackingState.TRACKED):
                ...
                draw_cross(int(left_hand_pos[0]), int(left_hand_pos[1]))
                draw_cross(int(right_hand_pos[0]), int(right_hand_pos[1]))
...
if __name__ == '__main__':
    ...
    grayscale = tuple([(i, i, i) for i in range(255)])
...
#        e = pygame.event.wait()
        ...
        #    data_int = data.view(numpy.uint8)
        #    print data_int.shape
        #    print data_int[40, 40]
        #    #depth_frame_values = [ ( (depth_frame_bits[i*2+1] << 8) + depth_frame_bits[i*2]) for i in range(len(depth_frame_bits)/2) ]
        #    #depth_frame_array = numpy.array(depth_frame_values)
        #    #print len(depth_frame_array)
        #    #depth_frame_array = depth_frame_array.reshape((DEPTH_WINSIZE))
        #    pygame.surfarray.blit_array(depth_surface, data_int.transpose())

        screen.blit(temp, (0,0))
        #screen.blit(depth_surface, (0,0))
        #screen.blit(temp2, (0,0), None, pygame.BLEND_SUB)
